...
public class State {
	...
	public State(final List<List<Tile>> view, final Vector2 startPos, final WumpusStartInfo startInfo,
	...
	/*
	 * Update the state. It takes the percept from the agent and the methode create
	 * a current view of the world.
	 * 
	 * If we can be sure there is something (e.g. wall) we set the tile type to
	 * this. Otherweise we add the tile a possible of a type.
	 * 
	...
	public void update(final Vector2 pos, final HunterPercept percept) {
		...
		if (percept.isBump())
			setPossibleTypeAround(pos, TileType.WALL);
		if (percept.isBreeze()) {
			setPossibleTypeAround(pos, TileType.PIT);
			getTile(pos).setBreeze(true);
		}

		// If wumpus percept is empty remove alle wumpus around hunter withhin stench
		// distance
			...
			for (List<Tile> l : this.view) {
				for (Tile n : l) {
					if (n == null)
						continue;
					// Manhatten distance
					int distance = Math.abs(pos.getX() - n.getPosX()) + Math.abs(pos.getY() - n.getPosY());
			...
			for (Map.Entry<Integer, Integer> id : percept.getWumpusStenchRadar().entrySet()) {
				...
				for (int y = hPosY - radius; y < hPosY + radius; y++) {
					for (int x = hPosX - radius; x < hPosX + radius; x++) {
						Tile n = getTile(x, y);
						int distance = Math.abs(hPosX - n.getPosX()) + Math.abs(hPosY - n.getPosY());
		...
		if (percept.isGlitter())
			getTile(pos).setTileType(TileType.GOLD);
	}

	public List<List<Tile>> getView() {
		return this.view;
	}
	 ...
	 * small "e" E = Empty H = Hunter W = Wall G = Gold X = Wumpus P = Pit B =
	 * Breeze U = Unkown (unproven tiles)
	 */
